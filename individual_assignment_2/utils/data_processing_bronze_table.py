...
def process_bronze_table(snapshot_date_str, bronze_root_directory, spark):
    ...
    for name, path in feature_sources.items():
        ...
        # 🟢 Filter rows belonging to current snapshot_date
        df_filtered = df.filter(col("snapshot_date") == snapshot_date)
    
        # 🟢 Create output directory (e.g., datamart/bronze/feature_clickstream/)
        output_dir = os.path.join(bronze_root_directory, f"feature_{name}/")
        os.makedirs(output_dir, exist_ok=True)
    
        # 🟢 Save only that month’s data as a new partition file
        out_file = os.path.join(
            output_dir,
            f"bronze_{name}_{snapshot_date_str.replace('-', '_')}.csv",
        )
    
        df_filtered.toPandas().to_csv(out_file, index=False)
        print(f"✅ Saved monthly {name} data → {out_file}")

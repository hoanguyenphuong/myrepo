...
def process_features_gold_table(snapshot_date_str, silver_base_directory, gold_feature_store_directory, spark):
    ...
    df_fin = df_fin.filter(col("snapshot_date") == F.lit(snapshot_date))
    df_attr = df_attr.filter(col("snapshot_date") == F.lit(snapshot_date))
    ...
    df_click_agg = (
        ...
        .groupBy("Customer_ID", "snapshot_date_fin")
        .agg(*[
            F.expr(f"percentile_approx({c}, 0.5)").alias(f"{c}_median")
            for c in click_cols
        ])
        .withColumnRenamed("snapshot_date_fin", "snapshot_date")
    )
    ...
    df_feat = df_fin.join(df_attr, on=["Customer_ID", "snapshot_date"], how="inner")

    # === Join aggregated clickstream ===
    df_feat = df_feat.join(df_click_agg, on=["Customer_ID", "snapshot_date"], how="left")
    ...
    # columns to exclude (raw clickstream + unwanted)
    exclude_cols = [c for c in df_feat.columns if c.startswith("fe_")]  # drop raw fe_ cols
    exclude_cols += ["Annual_Income"] if "Annual_Income" in df_feat.columns else []
    
    # keep everything else (including Customer_ID and snapshot_date)
    keep_cols = [c for c in df_feat.columns if c not in exclude_cols]
    
    print(f"Total columns kept: {len(keep_cols)}")
    print(f"Dropped columns: {len(exclude_cols)}")
    
    df_feat = df_feat.select(*keep_cols)

    # === Align snapshot_date for model training ===
    print("\n=== Aligning Feature Snapshot Dates (+6 months forward) ===")
    df_feat = df_feat.withColumn("snapshot_date", F.add_months(col("snapshot_date"), 6))

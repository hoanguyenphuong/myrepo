...
def process_silver_table(snapshot_date_str: str,
    ...
    fin_df = (fin_df
              ...
              .withColumn("Payment_Behaviour", when(col("Payment_Behaviour") == "!@9#%8", "Unknown").otherwise(col("Payment_Behaviour")).cast(StringType()))
              .withColumn("Credit_Mix", when(col("Credit_Mix") == "_", "Unknown").otherwise(col("Credit_Mix")).cast(StringType()))
              .withColumn("Type_of_Loan", when(col("Type_of_Loan").isNull(), "Unknown").otherwise(col("Type_of_Loan")).cast(StringType()))
              ...
              .withColumn("Amount_invested_monthly", regexp_replace("Amount_invested_monthly", "__", "").cast(FloatType()))
              .withColumn("Credit_History_Age", regexp_replace("Credit_History_Age", " Months", "").cast(FloatType()))
              .withColumn("Num_Credit_Card", col("Num_Credit_Card").cast(FloatType()))
